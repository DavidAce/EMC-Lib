...
double DNA::bin2dec(const int i) {
	...
	return value / (pow(2.0, geneLength) - 1) * (constants::upper_bound(i) - constants::lower_bound(i)) + constants::lower_bound(i);
}

bitset<geneLength> DNA::dec2bin(const int i) {
	double value = parameters(i);
	bitset <geneLength> A =  ((value - constants::lower_bound(i)) / (constants::upper_bound(i) - constants::lower_bound(i))*(pow(2.0, geneLength) - 1));
	return A;
}
...
void DNA::randomize_dna(){
    ...
    parameters.resize(nGenes);
    for (int i = 0; i < nGenes; i++) {
        parameters(i) = uniform_double(&rng, constants::lower_bound(i),constants::upper_bound(i));
        chromosomes[i] = dec2bin(i);
    }
}
...
DNA::DNA()  {
    ...
    cout << parameters.transpose() << endl;
    cout << constants::lower_bound.transpose() << endl;
    cout << constants::upper_bound.transpose() << endl;
}
